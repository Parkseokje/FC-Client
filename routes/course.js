    ...
    pool.getConnection((poolError, connection) => {
      ...
      async.series(
        [
          (callback) => {
            connection.query(
              QUERY.COURSE.SEL_INDEX,
              [trainingUserId, trainingUserId, courseId],
              (err, data) => {
                callback(err, data[0]);
              }
            );
          ...
          (callback) => {
            connection.query(
              QUERY.COURSE.SEL_SESSION_LIST,
              [req.user.user_id, trainingUserId, courseId],
              (err, data) => {
                callback(err, data);
              }
            );
      ...
      connection.beginTransaction((err) => {
        ...
        async.series(
          [
            ...
            (callback) => {
              connection.query(
                QUERY.EDU.UPD_TRAINING_USER_START_DT,
            ...
            (callback) => {
              connection.query(
                QUERY.COURSE.INS_COURSE_PROGRESS,
                [
                  _inputs.user_id,
                  _inputs.trainingUserId,
                  _inputs.courseId,
            ...
            (callback) => {
              if (_inputs.isrepeat) {
                connection.query(
                  QUERY.COURSE.UPD_COURSE_PROGRESS_REPEAT,
                  [_inputs.trainingUserId, _inputs.courseId],
                  (err, data) => {
                    callback(err, data);
                  }
                );
      ...
      async.series(
        [
          (callback) => {
            // 강의 종료일시 기록
            connection.query(
              QUERY.COURSE.UPD_COURSE_PROGRESS,
              [inputs.trainingUserId, inputs.courseId],
              (err, data) => {
                callback(err, data);
              }
            );
          ...
          (callback) => {
            // 강의 종료여부 조회
            connection.query(
              QUERY.COURSE.SEL_COURSE_END,
              [inputs.courseGroupId, inputs.trainingUserId],
              (err, data) => {
                if (data.length === 0) {
                  courseEndYn = true;
                }
          ...
          (callback) => {
            ...
            if (courseEndYn) {
              connection.query(
                QUERY.EDU.UPD_TRAINING_USER_END_DT,
        ...
        (callback) => {
          if (canAdvance !== 1) {
            ...
            if (prevCourseId === 0) {
              ...
              connection.query(
                QUERY.COURSE.GetCourseDone,
                [trainingUserId, prevCourseId],
                (err, data) => {
                  if (data !== undefined) {
