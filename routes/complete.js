    ...
    pool.getConnection((err, connection) => {
      ...
      async.series(
        [
          callback => {
            connection.query(
              QUERY.COURSE.SEL_COURSE_GROUP,
              [courseId, trainingUserId],
              (err, data) => {
                // data = undefined;
                courseGroup = data[0];
            ...
            connection.query(
              ...
              (err, data) => {
                if (data.length === 0) {
                  ...
                  connection.query(
                    QUERY.COURSE.SEL_NEXT_COURSE_2,
                    [courseGroup.group_id, courseId, courseGroup.order],
                    (err, data) => {
        ...
        (err, results) => {
          ...
          if (err) {
            ...
            res.render('complete', {
              ...
              current_url: req.url,
              root_url: req.user && req.user.root_path || '',
              host: req.get('origin'),
              loggedIn: req.user,
              header: '강의완료',
              training_user_id: trainingUserId,
