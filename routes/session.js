 ...
 */
router.get(
  '/:training_user_id/:course_id/:course_list_id',
    ...
    pool.getConnection((poolError, connection) => {
      ...
      async.series(
        [
          ...
          (callback) => {
            connection.query(
              QUERY.COURSE_LIST.SEL_INDEX,
              [trainingUserId, courseListId],
              (err, data) => {
                console.log(data);
                courseList = data[0];
          ...
          (callback) => {
            if (courseList.type === 'VIDEO') {
              connection.query(
                ...
                [req.user.user_id, trainingUserId, courseList.video_id],
                (err, data) => {
                  callback(err, data);
                }
              );
          ...
          (callback) => {
            if (courseList.type === 'VIDEO') {
              connection.query(
                QUERY.LOG_VIDEO.SEL_LAST_VIDEO_CURRENT_TIME,
                [req.user.user_id, trainingUserId, courseList.video_id],
                (err, data) => {
                  callback(err, data);
                }
          ...
          (callback) => {
            if (courseList.type === 'VIDEO') {
              connection.query(
                QUERY.LOG_COURSE_LIST.SEL_SESSION_PROGRESS,
                [req.user.user_id, trainingUserId, courseListId],
                (err, data) => {
                  callback(err, data);
                }
              );
        ...
        (err, results) => {
          ...
          if (err) {
            ...
            if (courseList.next_id) {
              ...
              nextUrl =
                '/' + 'evaluate' + '/' + trainingUserId + '/' + courseId;
            }
            ...
            if (courseList.type === 'VIDEO') {
              ...
              returnData.video_url =
                res.locals.vodPcUrl + returnData.content.url;
              returnData.watermark = req.user.user_id;
              returnData.total_played_seconds =
                results[2][0].total_played_seconds;
              returnData.setting = {
              ...
              if (videoType === 'vimeo') {
                ...
                if (returnData.total_played_seconds > 0) {
                  if (
                    Math.floor(results[2][0].max_duration * 0.8) <=
                    returnData.total_played_seconds
                  ) {
              ...
              courseList.type === 'QUIZ' ||
              courseList.type === 'FINAL'
            ) {
 ...
 */
router.post(
  '/player/log/:user_id/:training_user_id/:course_id/:course_list_id/:video_id/:total_played_seconds',
  (req, res, next) => {
    const inputs = {
    ...
    async.series(
      [
        ...
        (callback) => {
          connection.query(
            QUERY.LOG_COURSE_LIST.SEL_SESSION_PROGRESS,
            [inputs.user_id, inputs.training_user_id, inputs.course_list_id],
            (err, data) => {
              callback(err, data);
            }
          );
        ...
        (callback) => {
          connection.query(
            QUERY.LOG_COURSE_LIST.INS_SESSION_PROGRESS,
            [
              inputs.user_id,
...
router.post('/log/endtime', util.isAuthenticated, (req, res) => {
  ...
  pool.getConnection((poolError, connection) => {
    if (poolError) throw poolError;
    connection.query(
      QUERY.LOG_COURSE_LIST.UPD_SESSION_PROGRESS,
      [_inputs.user_id, _inputs.training_user_id, _inputs.course_list_id],
      (err, data) => {
        connection.release();
...
router.delete('/log', util.isAuthenticated, (req, res) => {
  ...
  pool.getConnection((poolError, connection) => {
    if (poolError) throw poolError;
    connection.query(
      QUERY.LOG_COURSE_LIST.INIT_SESSION_PROGRESS,
      [
